...
- [ ] Add strict models `ActionGraphDefinition`, `ActionGraphNode`, `ActionGraphEdge` and `ActionGraphAdmissionResult` with frozen fields from the spec.
- [ ] Reuse `ActionPrimitiveDefinition` references and existing semantic/policy validators; reject unknown refs, duplicate arrays, unbounded cycles, unreachable terminal nodes and missing recovery.
- [ ] Add tests for a valid graph, unknown primitive, duplicate node, cycle without bounded loop, conflicting edge and missing recovery; assert no event-store access occurs during content validation.
...
- [ ] Add strict `ActionWindowIntent`, `SpatialSnapshotRef`, `PerceptionResolution` and `ActionWindowResult` models; keep old `ActionIntent` unchanged.
- [ ] Implement a read-only validator that checks window ordering, graph/node membership, target scope, navigation/collision/occlusion/sound revisions and deterministic sample bounds.
- [ ] Add tests for valid movement, stale spatial revision, out-of-order window, changed duplicate, private evidence leakage and measurement conflict; all rejection paths must leave the store unchanged.
...
- [ ] Add graph-node playback selection, one-second window bookkeeping, interruption and recovery hooks to the existing controller; do not create a second controller.
- [ ] Add a presentation-only first-person camera toggle; retain the same actor, action attempt and committed projection identifiers.
- [ ] Ensure rejected windows clear speculative movement/action/panel state and return the controller to the last committed phase.
...
- [ ] Add a compatibility façade or additive methods that resolve one `ActionWindowIntent` into the bounded window event surface from the spec.
- [ ] Reuse current P5 registry, owner-fragment validation, `SettlementPlan`, append-derived receipt, privacy filtering and replay projector.
- [ ] Implement movement/visibility/sound/pursuit/contact/control/capture/retreat and case-terminal results as typed sub-results; reject client assertions that disagree with frozen spatial validation.
- [ ] Add tests for survivor/pursuer/witness roles, visibility isolation, duplicate/changed duplicate, stale revisions, contact conflict and no partial append.
...
- [ ] Add fixed fragment contracts for Body recovery/control, Inventory condition, Quest/Knowledge exposure, Social witness and Character/World death confirmation.
- [ ] Keep case death terminal to the encounter; require an explicit player/story confirmation before persistent death.
- [ ] Add zero-write tests for missing confirmation, stale source, private evidence, wrong owner fragment, duplicate and changed duplicate.
...
- [ ] Build the three-room reference scene from existing Godot primitives and reviewed affordance bridges; add hide spots, occluders, sound zones, door and clue without external art dependencies.
- [ ] Expose committed action/encounter state, current phase, exposure, control, capture/escape and terminal outcome through a read-only projection.
- [ ] Add revisioned voice templates for preparing, detected, captured, escaped, rejected and returned states.
- [ ] Verify speculative state clears on rejection and committed state survives reconnect/replay.
- [ ] Run focused projection tests, the new Harness profile, Godot headless and desktop probes.
...
- [ ] Prove full replay and checkpoint-tail replay equality across at least three action windows, one control result, one escape/capture branch and one death confirmation accept/reject path.
- [ ] Prove tampered graph, spatial snapshot, perception sample, policy, event payload and checkpoint rejection.
- [ ] Run `python -m pytest -q`, `python -m compileall -q backend`, `git diff --check`, the action Harness, embodied Harness, P5 Harness and docs Harness.
- [ ] Record a completion audit that distinguishes implemented action platform from future full combat/sports capability; do not mark August INF A-D complete.
