...
| `adventure-basic` | Implemented bounded closure gate. The matching [Adventure-Basic Reference Closure Plan](../../../plans/world-character-siming-authority-mainline/character-gameplay-foundation/2026-07-29-adventure-basic-reference-closure-plan.md), [.harness/verification/adventure-basic-report.json](../../../../../.harness/verification/adventure-basic-report.json), and [.harness/verification/live-adventure-basic-mirror-report.json](../../../../../.harness/verification/live-adventure-basic-mirror-report.json) prove the governed manifest plus the five approved end-to-end reference scenarios with rebuilt facade, online/full/checkpoint-tail replay, explanation trace, filtered mirror source, and real Godot mirror delivery after canonical authority commits. This row does not prove Patch activation, client authority, generic transport durability, persistence/migration closure, or broader domain completion. |
| `gameplay-foundation-all` | Implemented fail-closed aggregate for the currently implemented Gameplay Foundation dependency chain. [.harness/verification/gameplay-foundation-all-report.json](../../../../../.harness/verification/gameplay-foundation-all-report.json) proves only what each child profile explicitly proves and does not upgrade partial patch, mirror, or Godot scopes into broader generic closure. Broad repository completion still uses the top-level `all` profile. |
