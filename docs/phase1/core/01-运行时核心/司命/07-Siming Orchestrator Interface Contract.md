...
- `conversation_access_audit_result`
...
- `tick(sim_tick_ts: float, world_ts: float) -> void`
...
其中 `conversation_access_auditor` 不得隐含在 `participation_auditor` 内。高负载时可以把某个 auditor 的结果标记为 `stale` 或 `degraded`，但 `FairnessStateSnapshot` 中仍必须保留对应维度对象，不得用缺字段表达“没算”。
