...
`FairnessStateSnapshot` 必须按领域对象而不是临时 JSON 示例实现。

### 4.1 必填字段

| 字段 | 类型 | 约束 |
| --- | --- | --- |
| `schema_version` | string | 必填，例如 `1.0` |
| `snapshot_id` | string | 必填，建议前缀 `fair_snap_` |
| `room_id` | string | 必填 |
| `world_ts` | number | 必填，房间世界时间，属于 payload/领域对象 |
| `sim_tick_ts` | number | 必填，司命 tick 时间 |
| `producer_ts` | number | 必填，生成该快照的服务端时间 |
| `correlation_id` | string | 必填，用于串 replay 链 |
| `causation_id` | string | 必填，指向触发本快照的事件或 tick |
| `scene_scope` | string[] | 必填，可为空数组 |
| `phase_marker` | string | 必填 |
| `global_imbalance_score` | number | 必填，`0.0-1.0` |
| `dominant_imbalance_type` | enum | 必填，见 6.2 |
| `affected_actor_ids` | string[] | 必填，可为空数组 |
| `recommended_intervention_band` | enum | 必填，见 6.4 |
| `intervention_urgency` | enum | 必填，见 6.5 |

### 4.2 可选字段

五个维度对象可以在 `Phase 1` 中只实现前三个，但未实现维度必须显式写入：

```json
{
  "implemented": false,
  "reason": "phase1_deferred"
}
```
...
- 同一 `room_id + sim_tick_ts + causation_id` 不应生成多份语义不同的快照。
- late event 导致的补算必须生成新 `snapshot_id`，并在 `imbalance_reasons` 中标记 `late_input=true`。
- 快照只描述司命判断，不得写入角色信念真值或 ESM 物理真值。

## 5. 五个核心维度

### 5.1 `information_distribution`
...
### 5.2 `participation_distribution`
...
### 5.3 `conversation_access_fairness`
...
### 5.4 `suspicion_heat_distribution`
...
### 5.5 `evidence_visibility_distribution`
...
### 6.3 `affected_actor_ids`

保存当前受影响最大的角色/玩家集合。

### 6.4 `recommended_intervention_band`
