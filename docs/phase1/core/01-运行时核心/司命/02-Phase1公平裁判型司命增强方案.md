...
本文档在现有 [司命设计文档.md](../司命设计文档.md) 基础上，补齐《开本》在角色智能体、事件总线、Godot 本地执行、视觉事实系统都已复杂化之后，`Phase 1` 单局司命仍缺失的运行时骨架。
...
这 5 个对象构成 `Phase 1` 的最小公平裁判链。完整叙事投影、事件链生成、戏剧优先级模型和持续离线运行不进入 `Phase 1` 最小验收。

## 9. Phase 0 兼容边界

当前仓库的 `Phase 0 / Phase 0.5` 司命仍然可以只是一个后端权威的高层催化结果：

- 可以只输出 `attention_prompt` 或等价 high-level catalyst
- 不要求 `FairnessStateSnapshot`
- 不要求多 auditor
- 不要求 Redis / PostgreSQL 司命表
- 不要求工作台
- 不要求持续 tick
- 不要求叙事投影或事件链搜索

这些 Phase 1 文档不能倒逼 Phase 0 demo 提前实现完整司命系统。Phase 0 的验收仍以项目 harness 和运行时证据为准。
