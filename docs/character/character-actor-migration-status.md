...
- `CharacterModelGateway` now carries prompt policy and structured-output validation, and `L2/L3` now consume model-backed runtime outputs with local fallback preserved
...
- `CharacterReplica._on_character_agent_execution_received(...)` now also reads the normalized ingress `action` through `CharacterControllerPort.get_action_name(frame)` instead of continuing to fall back to shell-local payload-string access for that shared intent field
