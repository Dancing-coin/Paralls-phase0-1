...
- `CharacterRuntimeState` now also owns the remaining player-side requested-action / physiology presentation fields used by `build_player_presentation_input(...)`, so `CharacterReplica` no longer keeps those two presentation-state locals as shell-owned truth
...
- `Phase0ViewAnchorResolver` now also resolves the shared actor child through `PlayerShell.get_character_replica()` instead of direct wrapper tree lookup, and the older nested `CharacterReplica/VisualRoot` fallback has now been removed from the resolver path
...
- low-value player combat migration traces (`player_shell_mouse_button` / `mouse_button_state` / `combat_mouse_event` / `player_combat_action` / `phase0_input_bridge_ready`) are now removed from the default runtime/debug path; the remaining combat debug surface stays focused on higher-signal `role_action_overlay` evidence behind the same explicit bus toggle
...
- removal or explicit debug-gating of the remaining migration diagnostics beyond the already-trimmed player combat mouse traces
