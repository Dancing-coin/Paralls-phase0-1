...
三角色 W0/W1 仅作为回归 fixture。生产群体模拟通过通用候选/决策面，
由司命在多个已登记行为候选之间按压力、玩家邻近度、Owner 后果和预算
进行权衡；规则负责分类与约束，Owner 和 Character Core 仍负责最终写入。
该扩展不解除完整人口、社会、文明、多区域模拟的未完成状态。

## 领域 Owner 适配运行时状态（2026-09）

通用决策目录现在包含 Production/Organization、Inventory、public Social 与 Tax pressure
descriptor（能力描述符）。默认启动会读取仓库源码管理的 manifest，创建并激活共享
`GameplayPatchRegistry`（Gameplay 包注册表），再把同一个 registry 注入 Inventory、Social
和其他领域 Owner。当前 active patch set（已激活包集）已包含
`production_output_custody@1` 与 `population_signal_materialization@1`，因此
`population:organization-production-work-contribution:v1`、
`population:inventory-output-custody:v1` 和 `population:social-population-signal:v1`
进入默认 `owner_executors`；Bakery `schedule_gated_supply` 的 legacy `owner_executor`
继续保留。

Tax pressure 仅生成已脱敏的表现/激活候选，不注册 Tax Owner；Stormnight realtime action
window 属于实时案件交互，不注册为群体 cadence capability。manifest 缺失、digest 不匹配、
binding 不完整或 active patch set 无法组合时，启动链失败关闭，不伪造 package activation。

Tax pressure 的生产读取只能来自 Economy Authority 提供的窄 redacted projection（脱敏投影）：
`obligation_ref`、组织 scope、due/status 与 `gameplay:economy` revision pin。authority-only
原始事件、金额、账户、证据和 payment event 不进入群体 read-set；cadence 后只允许 report-only
disposition，不产生 Tax Owner receipt 或 Character Core receipt。

独立证据入口为 `siming-population-domain-owner-adaptation` Harness；它还验证 Owner receipt
先于 Character Core seed，以及 full/checkpoint-tail replay digest 一致。
