...
descriptor（能力描述符），但 descriptor 或 adapter 存在不等于默认生产 runtime 已获准
执行。默认 `owner_executors` 只注册
`population:organization-production-work-contribution:v1`，并继续保留 Bakery
`schedule_gated_supply` 的 legacy `owner_executor`。

Inventory 与 public Social adapter 已完成独立真实 Owner 测试；由于当前生产启动没有它们
各自合法的 active package binding（已激活包绑定），默认 runtime 必须 fail-closed
（失败关闭）为 `capability_owner_adapter_missing`、requeue、zero-write。Tax pressure 仅生成
已脱敏的表现/激活候选，不注册 Tax Owner；Stormnight realtime action window 属于实时案件
交互，不注册为群体 cadence capability。只有后续正式提供 source-controlled manifest、
activation 与所需 Owner 依赖后，Inventory 或 public Social 才能进入默认映射。
