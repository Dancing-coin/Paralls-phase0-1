...
│  它拥有：本地场景树、角色/物体位置、遮挡、环境场、可达性、局部观察窗口。         │
│                                                                                 │
│  玩家交互面                                                                     │
...
│                                                                               │  │
│  确定性空间与观察底座                                                         │  │
│  scene / room / zone / actor position / occupancy / environment field / LOS   │  │
│  local observation windows / body state / camera/listener frame               │  │
│                │                                                              │  │
│                ├───────────────┐                          ┌───────────────────┴─┐│
│                v               v                          v                     ││
│     ┌────────────────────┐  ┌──────────────────────┐  ┌──────────────────────┐ ││
│     │ 事实上抛器         │  │ spatial/access/      │  │ provider 采样器      │ ││
│     │ 抽取确定结论       │  │ observability 规则   │  │ 抽取 patch/ref/state │ ││
│     │ raw_fact_event     │  │ 并组织 fact family   │  │ 供 PQF / bundle 用   │ ││
│     └──────────┬─────────┘  └──────────────────────┘  └──────────┬───────────┘ ││
│                │                                                  │             ││
│                └──────────── raw_fact_event ──────────────────────┼──────────┐  ││
│                                                                   └ provider refs││
...
│  CharacterRuntimeState -> 角色建模皮肤 / voice stub / visible feedback       │ │
│                                                                                │ │
│  说明：                                                                       │ │
│  - `char_c` 与 `char_a/char_b` 共享 actor substrate，但控制源不同              │ │
│  - 玩家交互面只负责 `player_input` 类意图输出                                 │ │
...
│                                       ││ │                                      │
│  ┌────────────────────────────── 世界侧运行时域 ──────────────────────────────┐  │
│  │ world_runtime：世界侧运行时基础设施容器                                    │  │
│  │                                                                          │  │
│  │ ① L1 foundation                                                         │  │
│  │  Scene3DSpaceModel / SpatialOccupancyField / EnvironmentField /          │  │
│  │  FactProjectionLayer / fact_registry / continuity / scheduling           │  │
│  │                                                                          │  │
│  │ ② 事实上抛链路                                                          │  │
...
│  │                                                                          │  │
│  │ ④ VLA 慢路径                                                            │  │
│  │  PerceptionQueryFrame -> VLAProviderRequest -> VLAProviderResult ->      │  │
│  │  VLA percept bridge -> optional advisory merge                           │  │
│  │                                                                          │  │
│  │ ⑤ readiness / evidence                                                  │  │
│  │  model provider readiness / trace / conflict refs / replay support       │  │
│  └──────────────┬───────────────────────────────────────────┬───────────────┘  │
│                 │                                           │                  │
...
1. `Godot` 确定性空间层是本地空间/环境/观察底座；玩家交互面、事实上抛器、provider 采样器都只是从这个底座读取或提取不同类型的信息并跨层输出。
