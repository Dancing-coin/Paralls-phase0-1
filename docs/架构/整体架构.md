...
│  - 事实上抛器和采样器都属于 Godot 确定性空间层中的感知输出，不属于玩家交互输入  │
...
1. `Godot` 确定性空间层里包含两种不同性质的输出：玩家交互面产出 `player_input`，感知输出层产出 `raw_fact_event` 和 `provider refs`，三者都通过结构化 envelope 进入后端。
