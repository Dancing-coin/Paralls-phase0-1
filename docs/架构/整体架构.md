...
└─────────────────────────────────────────────────────────────────────────────────┘
                 ...
                 后端入口：route_raw_fact_event -> L1 world fact foundation ->
                          _messages_from_projected_l1_facts -> L1RuntimePerceptionBridge / ESM
...
│                                                                                 │
│  ┌──────────────────────────┐   ┌──────────────────────────────┐   ┌──────────────────────────┐ │
│  │ Authority 结算域         │   │ 世界侧运行时域 world_runtime │   │ 模型依赖摘要             │ │
│  │ 玩家意图 -> ESM ->       │   │                              │   │ Character / Siming / VLA │ │
│  │ settlement result        │   │ ① L1 foundation             │   │ provider readiness  
...
│  │ route / audit / replay   │   │    PQF -> VLA request ->   │                                      │
...
14. `ESM` 负责 authority settlement；结果先适配成 `AuthorityEvent`，再进入 `System L6`。
15. `System L6` 承载 authority event bus；`FrontendAuthorityEventProjector` 只负责前端投影，不拥有 Siming 或 ESM。
16. Godot 侧的 `CharacterReplica / CharacterRuntimeState / KnightRoleSkin` 不是一个东西，而是一条承接链：角色入口、状态承接、本地表现。
17. 模型服务侧栏是后端依赖能力，不是与 Godot/Backend 并列的第三运行层。
18. Harness 只证明实现、文档和集成状态，不替代运行时行为。
