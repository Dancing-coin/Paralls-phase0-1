...
└─────────────────────────────────────────────────────────────────────────────────┘
                 ...
                 后端入口：route_raw_fact_event / L1RuntimePerceptionBridge / ESM
...
│                                                                                 │
│  左侧：跨层公共系统                    中央：主运行时主体                    右侧：跨层依赖系统 │
│                                                                                 │
│  ┌──────────────────────────┐   ┌──────────────────────────────┐   ┌──────────────────────────┐ │
│  │ Authority 结算域         │   │ 世界侧运行时域 world_runtime │   │ 模型 Provider 能力域     │ │
│  │ 玩家意图 -> ESM ->       │   │                              │   │ Character text model     │ │
│  │ settlement result        │   │ ① L1 foundation             │   │ Siming candidate model   │ │
│  │ -> AuthorityEvent -> L6  │   │ ② 事实上抛链路              │   │ VLA provider             │ │
│  └──────────────┬───────────┘   │    RawFactEvent ->          │   │ readiness / adapter      │ │
│                 │               │    CharacterPerceivedEvent  │   │ fallback / proof         │ │
│                 │ public        │ ③ 多模态链路                │   └────────────┬─────────────┘ │
│                 │ AuthorityEvents│    provider refs ->        │                │ provider       │
│                 v               │    PerceptionInputFrame ->  │                │ capability     │
│  ┌──────────────────────────┐   │    PerceptionQueryFrame ->  │                │ / readiness    │
│  │ System L6 事件总线       │   │    base Canonical bundle   │                v                │
│  │ public authority bus     │   │ ④ VLA 慢路径增强支链       │   ┌──────────────────────────┐ │
│  │ route / audit / replay   │   │    PQF -> VLA request ->   │   │ 受控模型消费落点         │ │
│  │ frontend projection tap  │   │    VLA result -> merge back│   │ world_runtime /          │ │
│  └──────────────┬───────────┘   │ ⑤ readiness / evidence     │   │ Character / Siming       │ │
│                 │               └───────┬───────────┬────────┘   │ 不直接写 world truth      │ │
│                 │                       │           │            └──────────────────────────┘ │
│                 │                       │           │                                      │
│                 │                       │           ├──── global PQF / global bundle ───┐│
│                 │                       │           │                                    ││
...
│                 │          │   Siming catalyst            │  │ - global PQF / bundle       │ │
...
4. `Godot` 和后端之间应单独看“跨边界连接主线”：确定性空间层本身不过边界，真正过边界的是 `raw_fact_event`、`provider refs` 和 `player_input` 这类结构化 envelope；它们经 `BackendBridge.gd` 发到后端 `/ws`，由 `backend/app/main.py` 进入 `route_raw_fact_event` 和 `L1RuntimePerceptionBridge`。
5. 后端总图应该按“左侧跨层公共系统 / 中央主运行时主体 / 右侧跨层依赖系统”来读：事件总线放左侧，多模态主干留在中央，模型依赖留在右侧，这样不会把多模态链和事件总线误读成同一侧链路。
6. `world_runtime` 是中央主运行时主体的世界侧基础设施大类，内部同时承接事实上抛链和 provider/PQF 多模态链，并用显式 `PerceptionInputFrame` 做输入对齐。
7. `VLA` 不是与多模态链路并列的第二条主链，而是挂在 `PerceptionQueryFrame` 后面的慢速增强支链；它的结果只能通过 `VLA percept bridge` 回灌到 `CanonicalPerceptBundle`。
8. 角色侧运行时域和 Siming 全局态势域应处于水平并列层次：前者消费 actor-private `CharacterPerceivedEvent` 和角色 bundle，后者消费 `System L6` 公开事件以及来自两条感知链汇总出的全局 `PQF / bundle / public refs`。
9. 角色侧运行时要拆成输入层、内部链、输出层来看：输入是 `CharacterPerceivedEvent`、`CanonicalPerceptBundle`、`SelfBodyPerceivedEvent`、settlement summaries 和 `Siming catalyst`；内部经过 `L1 -> memory/timeline -> L2 -> L3 -> L4`；输出是 `character_agent_execution`、特定控制模式下的 `suggestion packets`，以及 actor-private writeback。
10. Siming 是独立的全局态势域，不应和 authority 结算或 frontend projection 混成一个盒子；它回写 `siming.*`，再由左侧跨层系统负责分发和投影。
11. Godot 侧也要分两类承接：`character_agent_execution` 进入 `CharacterReplica -> CharacterRuntimeState -> 角色建模皮肤`；`world_result / state_machine_transition / siming_output` 则进入 `LocalPresentationBus` 的公共表现分发，再落到语音桩和可见反馈。
12. `ESM` 负责 authority settlement；结果先适配成 `AuthorityEvent`，再进入 `System L6`。
13. `System L6` 承载 authority event bus；`FrontendAuthorityEventProjector` 只负责前端投影，不拥有 Siming 或 ESM。
14. Godot 侧的 `CharacterReplica / CharacterRuntimeState / KnightRoleSkin` 不是一个东西，而是一条承接链：角色入口、状态承接、本地表现。
15. 模型服务侧栏是后端依赖能力，不是与 Godot/Backend 并列的第三运行层。
16. Harness 只证明实现、文档和集成状态，不替代运行时行为。
