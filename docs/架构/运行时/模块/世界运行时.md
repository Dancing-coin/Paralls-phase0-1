...
│                 v                                       v                            │
│  ┌──────────────────────────────┐        ┌──────────────────────────────────────┐    │
│  │ 角色智能体 / Siming 消费方    │<───────│ model provider readiness refs        │    │
│  │ 只消费，不获得 world truth    │        │ 只记录 readiness，不证明真实成功调用 │    │
│  └──────────────┬───────────────┘        └──────────────────────────────────────┘    │
│                 │                                                                    │
│                 v                                                                    │
...
flowchart TD
    ...
    Bundle --> Character[角色智能体]
    Bundle --> Siming[Siming]
```
...
| 角色智能体 | `CharacterPerceivedEvent`、PQF 派生 bundle、参考性 metadata | 不写角色私有记忆，不选择角色 intent |
| ESM 与交互编排 | projected facts、空间/可达性相关上下文 | 不做 authority settlement |
| VLA 运行时通道 | PQF 和 provider refs | 不让 VLA 直接覆盖 world truth |
