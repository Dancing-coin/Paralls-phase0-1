...
func emit_odor_state_fact(target_environment_id: String, intensity_band: String = "noticeable", producer_ts: int = -1) -> bool:
	...
	return _get_raw_fact_emitter().emit_raw_fact(
		payload,
		"phase0_olfactory_fact_emitter",
		"phase0_olfactory_fact_emitter:odor_state_changed:%s" % intensity_band
	)
