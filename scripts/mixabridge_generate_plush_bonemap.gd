...
func _initialize() -> void:
	...
	if bone_map == null:
		push_error("mixabridge_generate_plush_bonemap: failed to create BoneMap")
		quit(1)
		return

	var save_err := mapper.save_bone_map(bone_map, OUTPUT_BONEMAP)
	if save_err != OK:
		push_error("mixabridge_generate_plush_bonemap: failed to save BoneMap: %s" % save_err)
		quit(2)
		return
	...
	if not mapper.detected_prefix.is_empty():
		print("DETECTED_PREFIX:%s" % mapper.detected_prefix)
	quit(0)
