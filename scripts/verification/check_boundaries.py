...
def _scan_active_omx_references(project_root: Path) -> list[str]:
    search_roots = [
    ...
    for root in search_roots:
        ...
        for path in paths:
            ...
            if "__pycache__" in path.parts:
                continue
            if path.suffix not in {".gd", ".md", ".py", ".toml"}:
                continue
...
def evaluate_boundaries(project_root: Path) -> dict[str, object]:
    ...
    omx_references = _scan_active_omx_references(project_root)
    results = [
        _result(
            "docs_index_exists",
        ...
        _result(
            ...
            "Harness artifacts use the project-local .harness directory instead of .omx paths",
            _contains(common, ['".harness"', '"verification"']) and not omx_references,
            ["scripts/verification/common.py"],
            "\n".join(omx_references),
        ),
        _result(
