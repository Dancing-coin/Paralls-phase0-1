...
def main() -> int:
    ...
    report = {
        ...
        "checks": checks,
        "focused_test_files": [
            artifact_ref(root, owner_tests).replace("\\", "/"),
            artifact_ref(root, semantic_tests).replace("\\", "/"),
            artifact_ref(root, owner_contract_tests).replace("\\", "/"),
            artifact_ref(root, lifecycle_contract_tests).replace("\\", "/"),
            artifact_ref(root, adapter_contract_tests).replace("\\", "/"),
        ],
        "evidence": evidence,
