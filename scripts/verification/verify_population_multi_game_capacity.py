...
def main():
    ...
    if args.verify_artifacts:
        ...
        directory = args.output or ROOT / '.harness/verification' / ('population-multi-game-'+datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%S%fZ'))
        result = collect_matrix(directory, expected_commit=revision)
        write_json(ROOT / '.harness/verification/population-multi-game-capacity-report.json', result)
