...
def main() -> int:
    ...
    for key,test in cases.items():
        log=verification_dir(root)/f"infra-semantic-ecology-frost-adapter-{key}.log"; result=run_command([python,"-m","pytest","-q",str(path),"-k",test],root,log); checks[key]=result.returncode==0; logs.append(artifact_ref(root, log).replace("\\","/"))
    report={"profile":"infra-semantic-ecology-frost-adapter","overall_passed":all(checks.values()),"checks":checks,"focused_test_files":[artifact_ref(root, path).replace("\\","/")],"evidence":logs,"run_id":f"infra-semantic-ecology-frost-adapter-{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')}","commit":evidence_revision(root),"write_path":"closed semantic proposal -> Ecology-owned envelope -> EcologyHazardAuthority.apply_crop_state -> one append -> outbox/replay","limitations":["Only effect:frost -> state:frosted@1 is admitted.","No generic ecology adapter or caller-selected owner/stream/event is admitted."]}
