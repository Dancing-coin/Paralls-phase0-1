...
def main() -> int:
    ...
    root = repo_root()
    scenario_runner = StormnightScenarioRunner()
    scenario_result = scenario_runner.run(outcome_kind="case_solved")
    projection_path = verification_dir(root) / "stormnight-case-committed-projection.json"
    committed_projection = scenario_runner.case.project().model_dump(mode="json")
    committed_projection["voice_state"] = "solved" if scenario_result.outcome_kind == "case_solved" else scenario_result.outcome_kind
    write_json(projection_path, committed_projection)
    os.environ["STORMNIGHT_CASE_PROJECTION_PATH"] = str(projection_path)
    ...
    desktop_log = verification_dir(root) / "stormnight-copper-sanatorium-desktop.log"
    ...
    if args.godot_exe:
        ...
        desktop_result = run_command([args.godot_exe, "--path", str(root), "--scene", "res://scenes/phase0/StormnightCopperSanatorium.tscn", "--quit-after", "2"], root, desktop_log)
        godot_ok = godot_ok and desktop_result.returncode == 0
    report = {"overall_passed": result.returncode == 0 and godot_ok, "focused_pytest_passed": result.returncode == 0, "godot_runtime_verified": godot_ok, "scenario_outcome": scenario_result.outcome_kind, "scenario_outcome_committed": scenario_result.outcome_committed, "scenario_projection_path": str(projection_path), "pytest_log": str(log), "godot_log": str(godot_log), "desktop_log": str(desktop_log), "godot_artifact": str(godot_artifact)}
