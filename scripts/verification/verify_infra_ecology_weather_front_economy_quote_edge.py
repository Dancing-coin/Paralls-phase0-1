...
def main():
 ...
 write_json(verification_dir(root)/"infra-ecology-weather-front-economy-quote-edge-report.json",{"profile":"infra-ecology-weather-front-economy-quote-edge","overall_passed":all(checks.values()),"checks":checks,"focused_test_files":[artifact_ref(root, test).replace("\\","/")],"evidence":[artifact_ref(root, verification_dir(root)/f"infra-ecology-weather-front-economy-quote-{case}.log").replace("\\","/") for case in cases]}); return 0 if all(checks.values()) else 1
