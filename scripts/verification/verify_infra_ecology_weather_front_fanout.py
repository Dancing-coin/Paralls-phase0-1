...
def main() -> int:
    ...
    for check, test_name in cases.items():
        ...
        checks[check] = result.returncode == 0; evidence.append(artifact_ref(root, log_path).replace("\\", "/"))
    report = {
        "profile": "infra-ecology-weather-front-fanout", "overall_passed": all(checks.values()), "checks": checks,
        "focused_test_files": [artifact_ref(root, test_path).replace("\\", "/")], "evidence": evidence,
