...
def evaluate_change_lifecycle(project_root: Path) -> dict[str, object]:
    ...
    results = [
        ...
        _result(
            "goal_replaces_omx_runtime_state",
            "Goal is the project workflow state surface and OMX is retired",
            all(marker in workflow_text for marker in ["OMX is retired", "Do not write new legacy OMX workflow state", "create_goal", "update_goal"])
            and "OMX is retired" in agents_text
            and legacy_omx_state_marker not in agents_text,
        ...
        _result(
            ...
            _contains(agents_md, ["docs/ai-engineering-workflow.md", "Goal", "Superpowers", "native subagents", "OMX is retired"]),
            ["AGENTS.md"],
        ),
    ]
    return {
