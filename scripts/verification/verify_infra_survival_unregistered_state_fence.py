...
def main() -> int:
    ...
    report = {"profile": "infra-survival-unregistered-state-fence", "overall_passed": result.returncode == 0, "checks": {"unregistered_reject_state_zero_write": result.returncode == 0}, "focused_test_files": [str(tests.relative_to(root)).replace("\\", "/")], "evidence": [str(log.relative_to(root)).replace("\\", "/")], "run_id": f"infra-survival-unregistered-state-fence-{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')}", "commit": evidence_revision(root), "limitations": ["No reject owner row is admitted."]}
