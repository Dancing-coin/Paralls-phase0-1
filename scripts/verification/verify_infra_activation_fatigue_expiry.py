...
def main() -> int:
    ...
    for check, selector in selectors.items():
        ...
        checks[check] = result.returncode == 0; evidence.append(artifact_ref(root, log).replace("\\", "/"))
    report = {"profile": "infra-activation-fatigue-expiry", "overall_passed": all(checks.values()), "checks": checks, "focused_test_files": [artifact_ref(root, tests).replace("\\", "/")], "evidence": evidence, "run_id": f"inf2n-{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')}", "commit": evidence_revision(root), "owner": "SurvivalAuthority", "write_path": "released activation pending -> existing Survival fragment -> one append_batch -> scoped projection/replay", "limitations": ["Only state:fatigued is additionally bound.", "No generic activation-obligation binding is added."]}
