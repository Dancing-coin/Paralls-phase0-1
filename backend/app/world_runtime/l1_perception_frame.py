...
class L1PerceptionFrameService:
    def build_character_frame(
        ...
        *,
        ...
        monotonic_tick = ended_at if monotonic_tick is None else monotonic_tick
        wall_clock_ts = ended_at if wall_clock_ts is None else wall_clock_ts
        capture_root_id = capture_root_id or derive_capture_root_id(
        ...
        capture_id = capture_id or derive_capture_id(
        ...
        target_ref = target_ref or first_target_ref(
            target_actor_ids=attention_target_actor_ids or [],
            target_object_ids=attention_target_object_ids or [],
            target_environment_ids=[],
        )
        source_lineage = append_unique_lineage(source_ref_lineage, structured_fact_refs or [])
        return PerceptionQueryFrame(
            ...
            **capture_kwargs,
            ...
            visual_inputs=self._inherit_capture_clock(visual_inputs or [], **capture_kwargs),
            spatial_inputs=self._inherit_capture_clock(spatial_inputs or [], **capture_kwargs),
            auditory_inputs=self._inherit_capture_clock(auditory_inputs or [], **capture_kwargs),
            embodied_inputs=self._inherit_capture_clock(embodied_inputs or [], **capture_kwargs),
            skeletal_inputs=self._inherit_capture_clock(skeletal_inputs or [], **capture_kwargs),
            environment_inputs=self._inherit_capture_clock(environment_inputs or [], **capture_kwargs),
