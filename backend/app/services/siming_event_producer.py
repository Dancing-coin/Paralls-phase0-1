...
class SimingEventProducer:
    def __init__(
        self,
        bus: AuthorityEventBusPort | None = None,
        *,
        authority_event_bus: AuthorityEventBusPort | None = None,
    ) -> None:
        ...
        *,
        ...
        event = AuthorityEvent(
            ...
            source=AuthorityEventSource(
                layer="L2", system="siming.dispatcher", actor_id=None
            ),
    ...
    def _to_authority_event(self, output: SimingOutput) -> AuthorityEvent:
        ...
        if (
            ...
            and not output.payload.get("established_fact_id")
        ):
            raise ValueError(
                "visual observability requests require established_fact_id"
            )
        if output.selected_path == "character_input_path":
            target_actor_id = str(
                output.payload.get("target_actor_id", "") or ""
            ).strip()
            if target_actor_id == "":
        ...
        return AuthorityEvent(
            ...
            source=AuthorityEventSource(
                layer="L2", system=self._source_system_for(output), actor_id=None
            ),
            routing=AuthorityEventRouting(
                audience_mode="targeted"
                if output.selected_path not in (None, "no_action")
                else "audit",
                routing_mode="event_type",
    ...
    def _event_type_for(self, output: SimingOutput) -> str:
        ...
        if output.output_type == "staging_request":
            return "siming.staging_request"
        ...
        if (
            output.selected_path == "character_input_path"
            and output.intervention_band == "impulse"
        ):
            return "siming.impulse"
        if (
            output.selected_path == "character_input_path"
            and output.intervention_band == "opportunity"
        ):
            return "siming.opportunity"
        if (
            output.selected_path == "character_input_path"
            and output.intervention_band == "fact_reveal"
        ):
            return "siming.fact_reveal"
        raise ValueError(
            f"unsupported Siming output mapping: {output.output_type}/{output.selected_path}/{output.intervention_band}"
        )
    ...
    def _target_ids_for(self, output: SimingOutput) -> list[str]:
        ...
        if output.selected_path == "character_input_path":
            return [
                str(output.payload.get("target_actor_id", "") or "").strip(),
                "frontend_projector",
            ]
        return ["audit"]
