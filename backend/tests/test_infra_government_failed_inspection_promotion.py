...
def _setup() -> tuple[GameplayEventStore, str, str]:
    ...
    branch_ref = "branch:government:failed-promotion"
    candidate = BatchIntentCandidate(
        intent_ref="intent:branch:government:failed-promotion",
        profile_ref="character:char_a",
        intent_kind="inspection",
        payload={
            "organization_ref": "organization:bakery",
            "inspection_ref": "inspection:branch:failed-promotion",
            "jurisdiction_ref": "jurisdiction:bakery",
            "policy_digest": "sha256:policy:failed-promotion",
            "evidence_ref": "evidence:branch:failed-promotion",
            "passed": False,
        },
        priority=1,
        policy_revision="policy:failed-promotion",
        package_revision="package:failed-promotion",
        idempotency_key="intent:branch:government:failed-promotion",
        correlation_id="corr:branch:government:failed-promotion",
        source_ref="planner",
    ...
    accepted = authority.preview(
        request=BranchPreviewRequest(
            ...
            deterministic_seed="seed:failed-promotion",
            active_revision_refs=("mode:failed-promotion",),
            calibration_ref="calibration:failed-promotion",
            privacy_scope="creator_debug",
        ),
        dataset=ReferenceDataset(
            dataset_ref="dataset:failed-promotion",
            provenance="fixture",
            license_ref="license:permitted",
            schema_revision="1",
            digest="sha256:dataset:failed-promotion",
        ...
        calibration=CalibrationInput(
            calibration_ref="calibration:failed-promotion",
            dataset_ref="dataset:failed-promotion",
            parameter_mapping_revision="map:failed-promotion",
            world_revision="world:failed-promotion",
            ruleset_revision="rules:failed-promotion",
            privacy_scope="creator_debug",
        ),
        family_inputs=(),
        ...
        mode=WorldModeProfile(
            world_ref="world:bakery",
            mode="preview",
            revision="mode:failed-promotion",
            cadence_class="daily",
            batch_limit=1,
            wake_budget=1,
            catch_up_limit=1,
    ...
    scenario_receipt = authority.settle_accepted_failed_inspection_remediation_scenario(
...
def test_government_promotes_one_durable_failed_inspection_into_existing_production_stream() -> None:
    store, admission_event_id, scenario_event_id = _setup()
    promote = getattr(GovernmentAuthority(store=store), "promote_failed_branch_inspection", None)
    ...
    result = promote(
        ...
        idempotency_key="promotion:government:failed-inspection:1",
    ...
    assert event.payload["branch_ref"] == "branch:government:failed-promotion"
...
def test_government_failed_inspection_promotion_exact_duplicate_reconstructs_receipt_without_second_append() -> None:
    store, admission_event_id, scenario_event_id = _setup()
    government = GovernmentAuthority(store=store)
    promote = getattr(government, "promote_failed_branch_inspection", None)
    ...
    first = promote(
        ...
        scenario_event_id=scenario_event_id,
        expected_production_revision=0,
        idempotency_key="promotion:government:failed-inspection:duplicate",
        privacy_scope="project",
    )
    duplicate = promote(
        admission_event_id=admission_event_id,
...
def test_government_failed_inspection_promotion_rejects_changed_duplicate_without_append() -> None:
    store, admission_event_id, scenario_event_id = _setup()
    government = GovernmentAuthority(store=store)
    promote = getattr(government, "promote_failed_branch_inspection", None)
    ...
    assert promote(
        ...
        scenario_event_id=scenario_event_id,
        expected_production_revision=0,
        idempotency_key="promotion:government:failed-inspection:changed",
        privacy_scope="project",
    ).accepted
...
def test_government_failed_inspection_promotion_rejects_stale_production_source_without_append() -> None:
    store, admission_event_id, scenario_event_id = _setup()
    government = GovernmentAuthority(store=store)
    promote = getattr(government, "promote_failed_branch_inspection", None)
    ...
    assert government.settle_tax_assessment(
        organization_ref="organization:bakery",
        period_ref="period:failed-promotion-source-advance",
        revenue=10.0,
        rate=0.1,
        policy_revision="policy:failed-promotion-source-advance",
        command_id="command:failed-promotion-source-advance",
        idempotency_key="idempotency:failed-promotion-source-advance",
        causation_id="cause:failed-promotion-source-advance",
        correlation_id="corr:failed-promotion-source-advance",
    ).committed
    ...
    rejected = promote(
        ...
        scenario_event_id=scenario_event_id,
        expected_production_revision=0,
        idempotency_key="promotion:government:failed-inspection:stale",
        privacy_scope="project",
    )
...
def test_government_failed_inspection_promotion_rejects_wrong_privacy_without_append() -> None:
    store, admission_event_id, scenario_event_id = _setup()
    promote = getattr(GovernmentAuthority(store=store), "promote_failed_branch_inspection", None)
    ...
    rejected = promote(
        ...
        scenario_event_id=scenario_event_id,
        expected_production_revision=0,
        idempotency_key="promotion:government:failed-inspection:privacy",
        privacy_scope="creator_debug",
    )
...
def test_government_failed_inspection_promotion_rejects_forged_scenario_identity_without_append() -> None:
    ...
    rejected = promote(
        ...
        expected_production_revision=0,
        idempotency_key="promotion:government:failed-inspection:forged",
        privacy_scope="project",
    )
...
def test_government_failed_inspection_promotion_rejects_catalog_mismatch_before_production_append(
    monkeypatch,
) -> None:
    store, admission_event_id, scenario_event_id = _setup()
    promote = getattr(GovernmentAuthority(store=store), "promote_failed_branch_inspection", None)
    ...
    rejected = promote(
        ...
        scenario_event_id=scenario_event_id,
        expected_production_revision=0,
        idempotency_key="promotion:government:failed-inspection:catalog-mismatch",
        privacy_scope="project",
    )
...
def test_government_failed_inspection_promotion_emits_scoped_outbox_and_replays_production_checkpoint_tail() -> None:
    store, admission_event_id, scenario_event_id = _setup()
    ...
    assert promote(
        ...
        scenario_event_id=scenario_event_id,
        expected_production_revision=0,
        idempotency_key="promotion:government:failed-inspection:replay",
        privacy_scope="project",
    ).accepted
