...
def test_websocket_default_scene_stow_moves_picked_up_item_into_inventory_and_commits_three_events() -> None:
    ...
    assert carry_place_event["message_type"] == "embodied_carry_place_event"
    assert stow_ack["message_type"] == "ack"
    assert stow_ack["payload"]["accepted"] is True
    assert stow_ack["payload"]["source_type"] == "player_input"
    assert stow_ack["payload"]["route"] == "default_scene_inventory_authority"
    assert stow_ack["payload"]["intent_type"] == "stow_intent"
...
def test_websocket_default_scene_stow_rejects_when_item_is_not_in_actor_custody() -> None:
    ...
    assert ack["payload"]["accepted"] is False
    assert ack["payload"]["source_type"] == "player_input"
    assert ack["payload"]["route"] == "default_scene_inventory_authority"
    assert ack["payload"]["request_id"] == ""
    assert ack["payload"]["intent_type"] == "stow_intent"
    assert ack["payload"]["producer_ts"] == 201
