...
def _registry() -> SemanticRegistry:
    ...
    registry.register_state_lifecycle(
        StateLifecyclePolicy(
            state_ref="state:overheated", effect_ref="effect:heat_exposure", lifecycle="scheduled", revision="1",
...
def _submit(authority: SemanticSettlementAuthority, registry: SemanticRegistry, *, key: str = "heat:one", expected_revision: int = 0, privacy_scope: str = "project", effect_ref: str = "effect:heat_exposure"):
    ...
    return authority.settle_closed_survival_state(
        ...
        application=EffectApplication(effect_ref=effect_ref, target_component_ref="character:ava", magnitude=100, stack_key="heat", expires_at_tick=8, causal_chain_id="chain:heat:1"),
...
def test_overheated_owner_row_opens_existing_survival_obligation() -> None:
    ...
    assert SurvivalAuthority(store=store).projector().open_obligations == {"obligation:survival:state:character:ava:state:overheated": 8}


def test_overheated_owner_row_is_idempotent() -> None:
    store = GameplayEventStore()
    registry = _registry()
...
def test_overheated_owner_row_rejects_stale_revision_without_writes() -> None:
    store = GameplayEventStore()
    registry = _registry()
    authority = SemanticSettlementAuthority(store=store, registry=registry)
    assert _submit(authority, registry).committed
    ...
    stale = _submit(authority, registry, key="heat:stale", expected_revision=0)

    assert stale.failure is not None and stale.failure.error_code == "revision_conflict"
    assert len(store.read_events()) == before


def test_overheated_owner_row_rejects_nonproject_privacy_without_writes() -> None:
    store = GameplayEventStore()
    registry = _registry()

    rejected = _submit(SemanticSettlementAuthority(store=store, registry=registry), registry, privacy_scope="authority_only")

    assert rejected.failure is not None and rejected.failure.error_code == "semantic_survival_privacy_scope_denied"
    assert store.read_events() == []


def test_overheated_owner_row_rejects_unpaired_effect_without_writes() -> None:
    store = GameplayEventStore()
    registry = _registry()

    rejected = _submit(SemanticSettlementAuthority(store=store, registry=registry), registry, effect_ref="effect:cold_exposure")

    assert rejected.failure is not None and rejected.failure.error_code == "semantic_survival_owner_mapping_unregistered"
    assert store.read_events() == []


def test_overheated_expiry_settles_and_replays_checkpoint_tail() -> None:
    store = GameplayEventStore()
    registry = _registry()
    ...
    obligation = policy.build_obligation(
        actor_ref="character:ava", state_ref="state:overheated", due_tick=8,
        expected_revision=2, status="due",
    )
    ...
    plan = coordinator.plan_settle(
        ...
        fragments=(SurvivalAuthority.build_state_expiry_fragment(obligation=obligation, actor_ref="character:ava", state_ref="state:overheated", expected_revision=2),),
    ...
    assert result.committed is True
    assert ("character:ava", "state:overheated") not in SurvivalAuthority(store=store).projector().states
