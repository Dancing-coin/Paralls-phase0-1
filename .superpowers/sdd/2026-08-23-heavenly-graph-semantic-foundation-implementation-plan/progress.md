...
Final fix wave 1: scoped re-review rejected; fix wave 2 must remove the `closes_branch_node` endpoint-visibility bypass, apply branch availability/close visibility to historical ConflictSet reads, and bound lifecycle marker selection in branch diff.
Final fix wave 2: scoped re-review found marker selection bounded only by eligible matches. Fix wave 3 must cap total marker inspections for ineligible historical/policy streams and report conservative truncation on scan saturation.
Final fix wave 3: scoped re-review partial; same-scope branch diff scans the same marker stream twice. Fix wave 4 must reuse one bounded window when left/right scopes are identical and prevent duplicate markers.
Final fix wave 4: same-scope marker window reuse implemented in 53d9b93, with an additional same-scope regression committed in f1743d2; manual scoped inspection confirms one bounded window and unique marker output. Reviewer channel was unavailable (model/service errors), so final verification is the evidence gate.
